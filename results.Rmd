...
b_counts = as.data.frame(cbind(Year = summer_counts$Year,
...
```{r abundant_bird_212}
# apply jags analysis for red-knobbed coot
redKnobbedCoot = barberspan_counts %>% select(Year, Season, counts = `Red-knobbed Coot`)
redKnobbedJags = jags_analysis(redKnobbedCoot)

```

- SSM fitted plot for abundant bird
```{r plot_red_knobbed_coot}
redKnobbedPlot = ts_jag_plot(redKnobbedJags, redKnobbedCoot, "Jags analysis output for Red-Knobbed Coot")

redKnobbedPlot$summer
redKnobbedPlot$winter

```

- Plot of single bird species (less abundant) count fluctuation over time.
```{r rare_bird_Marsh_owl}
load('data/all_Barberspan_counts.RData')

# apply jags analysis for marsh owl
MarshOwl = all_barberspan_counts %>% select(Year, Season, counts = `Marsh Owl`)
MarshOwlJags = jags_analysis(MarshOwl)

```
- SSM fitted plot for less abundant bird

```{r plot_marsh_owl_jags}
MarshOwlPlot = ts_jag_plot(MarshOwlJags, MarshOwl, "Jags analysis output for Marsh Owl")
