...
b_counts = rowSums(barberspan_counts[3:43], na.rm = T) # sum across rows
...
The hyperparameters for number of iterations and burnin for the Gibbs Sampler was 10 000 and 5000 respectively. The number of iterations refers to the number of times the Gibbs Sampler samples from the probability distribution that represents the summer or winter counts. The "burn-in" hyperparameter refers to the first few iterations that are to be discarded given that these first few iterations are usually less probable outcomes, but as the model runs for a longer period of iterations, the outcomes tend toward a higher probability region and thus are more likely outcomes. This generates an output of 15 000 count values for each year with the first 5000 iterations being the "burn-in" iterations. The state-space time series model output is displayed in figure (x). The diagnostic output of the state-space time series model is presented in Appendix (B) where trace plots of each MCMC chain is visualised to illustrate the convergence in count values for each year.
...
```{r jags_per_species, results='hide', message=FALSE}
...
```
could instead show output of other parameters like lambda, beta, zeta, etc. Add their trace plots to Appendix B also along with trace plots of jags analysis on combined bird species
...
- MCMC output for each to show convergence (to compare how well the model fits in either situation) (add MCMC traceplots to the appendix)
